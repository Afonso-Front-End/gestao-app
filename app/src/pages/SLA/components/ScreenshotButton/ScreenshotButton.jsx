  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      try {
        ...
        useCORS: true,
        allowTaint: true,
        backgroundColor: null, // Preservar cores originais (transparente)
        logging: false,
        imageTimeout: 15000,
        removeContainer: true,
        foreignObjectRendering: false, // Melhor preservação de cores
          ...
          clonedDoc.querySelectorAll('*').forEach((el) => {
            if (el !== clonedElement) {
              ...
              if (el.classList && (
                ...
                el.classList.contains('sla-overlay-table-wrapper')
              )) {
                el.style.margin = '0'
                el.style.padding = '0'
                el.style.width = 'auto'
              ...
              // Ajustar sla-table-header (preserva TODAS as cores e estilos)
              if (el.classList && el.classList.contains('sla-overlay-header')) {
                // Forçar reset de margins mas preservar TODOS os outros estilos
                el.style.margin = '0'
                el.style.marginTop = '0'
                el.style.marginBottom = '0'
              ...
              if (el.classList && (
                ...
                el.classList.contains('sla-overlay-title-section') ||
                el.classList.contains('sla-overlay-subtitle')
              )) {
                el.style.animation = 'none'
                el.style.transition = 'none'
                el.style.transform = 'none'
              ...
              // Preservar padding dos elementos sla-pill
              if (el.classList && el.classList.contains('sla-pill')) {
                // Não remover padding - preservar o estilo original
              ...
              // Remover margin-right de elementos que podem criar espaços (exceto sla-pill)
              if (!el.classList || !el.classList.contains('sla-pill')) {
                const computedStyle = window.getComputedStyle(el)
      ...
      // Função para remover espaços em branco do canvas (preservando todas as cores)
      const trimCanvas = (c) => {
        ...
        // Encontrar bounds (top, left, right, bottom) - considerar qualquer pixel não totalmente transparente
        for (y = 0; y < c.height; y++) {
          for (x = 0; x < c.width; x++) {
            const alpha = data[((y * c.width + x) * 4) + 3]
            // Considerar qualquer pixel com alpha > 0 (não importa a cor)
            if (alpha > 0) {
        ...
        // Se encontrou conteúdo, cropar preservando todas as cores
      ...
      // Remover espaços em branco (preservando todas as cores)
      const trimmedCanvas = trimCanvas(canvas)
      
      // Garantir máxima qualidade de cores no PNG (1.0 = qualidade máxima, preserva todas as cores)
