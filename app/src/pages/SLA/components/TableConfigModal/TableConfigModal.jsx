...
const TableConfigModal = ({ 
  ...
  // Ordenar colunas por order para garantir ordem correta (memoizado)
  ...
  const handleDrop = useCallback((e, dropIndex) => {
    ...
    setDraggedIndex(null)
    setDragOverIndex(null)
  }, [draggedIndex, sortedColumns, onUpdateOrder])

  const handleStyleChange = useCallback((columnId, styleKey, value) => {
    onUpdateStyles(columnId, { [styleKey]: value })
  }, [onUpdateStyles])

  const getStylePreview = useCallback((styles) => {
    return {
  ...
  return (
    <div className={`sla-table-config-modal-overlay ${isClosing ? 'fade-out' : 'fade-in'}`} onClick={handleClose}>
      <div className={`sla-table-config-modal ${isClosing ? 'slide-down' : 'slide-up'}`} onClick={(e) => e.stopPropagation()}>
        <div className="sla-table-config-modal-header">
          <h2>Configurações da Tabela</h2>
          <button className="sla-table-config-modal-close" onClick={handleClose}>
            <FaTimes />
          </button>
        </div>

        <div className="sla-table-config-modal-tabs">
          <button
            className={`sla-table-config-tab ${activeTab === 'columns' ? 'active' : ''}`}
            onClick={() => setActiveTab('columns')}
          >
            <FaGripVertical /> Colunas
          </button>
          <button
            className={`sla-table-config-tab ${activeTab === 'styles' ? 'active' : ''}`}
        ...
        <div className="sla-table-config-modal-content">
          {activeTab === 'columns' && (
            <div className="sla-table-config-columns">
              <div className="sla-table-config-section-header">
              ...
              {onResetColumns && (
                <div className="sla-table-config-section-reset">
                  <button 
                    className="sla-table-config-reset-section-btn" 
              ...
              <div className="sla-table-config-columns-list">
                {sortedColumns.map((column, index) => {
                  ...
                  <div
                    key={column.id}
                    className={`sla-table-config-column-item ${
                  ...
                  >
                    <div className="sla-table-config-column-drag-handle">
                      <FaGripVertical />
                    </div>
                    <div className="sla-table-config-column-name">
                      {column.name}
                    </div>
                    {onUpdateFixedColumns && (
                      <button
                        className={`sla-table-config-column-fixed ${
                    ...
                    <button
                      className={`sla-table-config-column-visibility ${
          ...
          {activeTab === 'styles' && (
            <div className="sla-table-config-styles">
              <div className="sla-table-config-section-header">
              ...
              <div className="sla-table-config-styles-list">
                {sortedColumns.map((column) => (
                  <div key={column.id} className="sla-table-config-style-item">
                    <div className="sla-table-config-style-header">
                      <h4>{column.name}</h4>
                      <div 
                        className="sla-table-config-style-preview"
                    ...
                    <div className="sla-table-config-style-controls">
                      <div className="sla-table-config-style-group">
                        <label>Cor de Fundo:</label>
                        <div className="sla-table-config-color-palette">
                          {predefinedColors.background.map((color) => (
                            <button
                              key={color.value}
                              type="button"
                              className={`sla-table-config-color-swatch ${
                        ...
                        <div className="sla-table-config-color-inputs">
                          <input
                            type="color"
                      ...
                      <div className="sla-table-config-style-group">
                        ...
                        <div className="sla-table-config-font-weight-buttons">
                          <button
                            className={`sla-table-config-font-btn ${
                      ...
                      <div className="sla-table-config-style-group">
                        ...
                        <div className="sla-table-config-font-style-buttons">
                          <button
                            className={`sla-table-config-font-btn ${
        ...
        <div className="sla-table-config-modal-footer">
          <button 
            className="sla-table-config-reset-btn" 
            onClick={() => {
          ...
          <button className="sla-table-config-close-btn" onClick={handleClose}>
            Fechar
          </button>
        </div>
      </div>
