...
const TableFilterBar = ({ columns, data, onFilterChange }) => {
  ...
  return (
    <div className="d1-table-filter-bar">
      <div className="d1-table-filter-bar-header">
        <button
          className={`d1-table-filter-toggle ${isExpanded ? 'expanded' : ''}`}
        ...
        >
          ...
          {activeFiltersCount > 0 && (
            <span className="d1-table-filter-badge">{activeFiltersCount}</span>
          )}
        </button>
        {activeFiltersCount > 0 && (
          <button
            className="d1-table-filter-clear-all"
      ...
      {isExpanded && (
        <div className="d1-table-filter-content">
          <div className="d1-table-filter-grid">
            {visibleFilterableColumns.map(columnName => {
              ...
              return (
                <div key={columnName} className="d1-table-filter-item">
                  <div className="d1-table-filter-header">
                    <label className="d1-table-filter-label">
                      {columnName}
                      {hasFilter && (
                        <span className="d1-table-filter-count">({selectedValues.size})</span>
                      )}
                    </label>
                    <div className="d1-table-filter-actions">
                      {hasFilter && (
                        <>
                          <button
                            className="d1-table-filter-action-btn"
                          ...
                          <button
                            className="d1-table-filter-clear-btn"
                      ...
                      <button
                        className="d1-table-filter-expand-btn"
                  ...
                  {/* Input de pesquisa */}
                  <div className="d1-table-filter-input-wrapper">
                    <FaSearch className="d1-table-filter-search-icon" />
                    <input
                      type="text"
                      className="d1-table-filter-input"
                    ...
                    {searchTerm && (
                      <button
                        className="d1-table-filter-clear-search"
                  ...
                  {isColumnExpanded && (
                    <div className="d1-table-filter-values">
                      {filteredValues.length > 0 ? (
                        <>
                          <div className="d1-table-filter-values-list">
                            {filteredValues.map(value => {
                              const isSelected = selectedValues.has(value)
                              return (
                                <label
                                  key={value}
                                  className="d1-table-filter-value-item"
                                >
                                  <input
                                    type="checkbox"
                                    checked={isSelected}
                          ...
                          {filteredValues.length < values.length && (
                            <div className="d1-table-filter-values-info">
                              Mostrando {filteredValues.length} de {values.length} valores
                            </div>
                          )}
                        </>
                      ) : (
                        <div className="d1-table-filter-no-results">
          ...
          {activeFiltersCount > 0 && (
            <div className="d1-table-filter-info">
