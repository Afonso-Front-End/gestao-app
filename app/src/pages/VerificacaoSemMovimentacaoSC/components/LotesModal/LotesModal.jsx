...
const LotesModal = ({ isOpen, onClose, pedidosLotes, filteredData, onCopyLote }) => {
  ...
  const getStorageKey = useCallback((loteNumero) => {
    return `verificacao_sem_movimentacao_sc_lote_${loteNumero}_planilha`
  }, [])
    ...
    if (isOpen && pedidosLotes.length > 0) {
      ...
      pedidosLotes.forEach((lote) => {
        ...
        if (savedData && Object.keys(savedData).length > 0) {
          ...
          // Preencher automaticamente com os números dos pedidos
          lote.pedidos.forEach((pedido, index) => {
            const globalIndex = startIndex + index
            if (globalIndex < startIndex + 500) {
              planilhaDataRef.current[globalIndex] = {
                id: globalIndex + 1,
                ...defaultValues,
                numerosPedidos: pedido
              }
              needsUpdate = true
            }
          })
          ...
          for (let i = lote.pedidos.length; i < 500; i++) {
  ...
  }, [isOpen, pedidosLotes, defaultValues, loadFromStorage, saveToStorage])
  ...
  const handleExportExcel = useCallback(async (lote) => {
    try {
      ...
      link.download = `Lote_${lote.numero_lote}_VerificacaoSemMovimentacaoSC_${dateStr}.xlsx`
  ...
  const totalPedidos = pedidosLotes.reduce((sum, lote) => sum + lote.total_pedidos, 0)

  return (
    <div className={`lotes-modal-overlay ${isClosing ? 'closing' : ''}`} onClick={handleClose}>
      <div className="lotes-modal" onClick={(e) => e.stopPropagation()}>
        <div className="lotes-modal-header">
          <h2>📦 Lotes de 500 Pedidos</h2>
          <button
        ...
        <div className="lotes-modal-content">
          <p className="lotes-modal-info">
            Total: {totalPedidos.toLocaleString('pt-BR')} pedido(s) divididos em {pedidosLotes.length} lote(s) de 500
          </p>
            ...
            {pedidosLotes.map((lote) => (
              <div key={lote.numero_lote} className="lote-planilha">
                <h3 className="lote-planilha-title">
                  Lote {lote.numero_lote} - {lote.total_pedidos.toLocaleString('pt-BR')} pedido(s)
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                      className="lotes-btn-copy-lote-small"
                      onClick={() => onCopyLote && onCopyLote(lote.pedidos.join('\n'))}
                      title="Copiar números de pedidos"
                    >
                      📋 Copiar
                    </button>
                    <button
